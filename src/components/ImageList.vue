...
function getDisplayName(name, maxBaseChars = 20) {
  if (!name) return "";

  const dotIndex = name.lastIndexOf(".");
  if (dotIndex <= 0 || dotIndex === name.length - 1) {
    return name.length > maxBaseChars
      ? name.slice(0, maxBaseChars) + "..."
      : name;
  }

  const base = name.slice(0, dotIndex);
  const ext = name.slice(dotIndex);
  return base.length > maxBaseChars
    ? base.slice(0, maxBaseChars) + "..." + ext
    : name;
}
  ...
  <div v-if="images?.length">
    ...
    <ol class="imageList--list">
      <li v-for="img in images" :key="img.name" class="imageList--list--item">
        <div class="item--meta">
          <div class="item--title" :title="img.name">
            {{ getDisplayName(img.name) }}
          </div>
          <div class="item--sub">
            <template
              v-if="
                img.originalSize && img.size && img.size !== img.originalSize
              "
            >
              <small class="imageList--list--item--reduction"
