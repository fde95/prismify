...
function addFiles(newFiles) {
    ...
    for (const f of arr) {
        ...
        if (!files.value.find(x => x.file.name === f.name && x.file.size === f.size)) {
            const url = URL.createObjectURL(f)
            files.value.push({ file: f, url })
        }
    }
    emit('images-selected', files.value.map(x => x.file))
}
...
<template>
    <div class="uploader">
        <label class="dropzone" @drop="onDrop" @dragover="onDragOver" tabindex="0">
            <input type="file" multiple accept="image/*" @change="handleUpload" class="visually-hidden" />
            <div class="dropzone--content">
                <div class="dropzone--content--icon">
                    <img class="dropzone--content--icon--img" src="../assets/icon/upload.webp" alt="Envie suas imagens"
                        width="32" height="32" loading="lazy">
                </div>
                <div class="dropzone--content--description">
                    <span><strong>Arraste imagens aqui</strong></span>
                    <span><small>ou clique para selecionar — PNG, JPG, WEBP, GIF</small></span>
                </div>
            </div>
        </label>

        <div class="previews" v-if="files.length">
            <div v-for="(f, i) in files" :key="f.file.name + f.file.size" class="previews--preview"
                title="Clique para remover">
                <img :src="f.url" alt="preview" class="previews--preview--img" />
                <button class="remove" @click="removeAt(i)" aria-label="Remover imagem">✕</button>
                <div v-if="converting && converting.includes(f.file.name)" class="overlay">
